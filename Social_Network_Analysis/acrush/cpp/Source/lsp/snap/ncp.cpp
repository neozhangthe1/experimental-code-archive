...
int TLocClust::ApproxPageRank(const int& StartNId, const double& Eps) {
  ...
  if (community.size()>=30)
  {
	  FILE *f=fopen("community.txt","a");
	  fprintf(f,"%d",(int)community.size());
	  for (set<int>::iterator it=community.begin();it!=community.end();++it)
		  fprintf(f," %d",(*it)-1);
	  fprintf(f,"\n");
	  fclose(f);
  }
