    ...
    for (int p = 1; p < num_procs; p ++)
    {
        ...
        for (int i = start[p]; i < start[p + 1]; i ++)
        {
            ...
            for (int t = 0; t < p_sample->num_node; t ++)
            {
                ...
                WriteInt(mem, mem_p, p_node->label_type);
...
DataSet* Transmitter::GetJobs()
{
    ...
    for (int i = 0; i < dataset->num_sample; i ++)
    {
        ...
        for (int t = 0; t < p_sample->num_node; t ++)
        {
            ...
            p_node->label_type = ReadInt(mem, mem_p);
...
void Transmitter::Master_CollectGradientInfo(double* gradient, double* f, int num_feature, double* tmp_store, int num_procs)
{
    ...
    for (int p = 1; p < num_procs; p ++)
    {
        MPI_Recv(tmp_store, (num_feature + 1) * sizeof(double), MPI_BYTE, p, MSG_DOUBLE_ARR, MPI_COMM_WORLD, &status);
...
void Transmitter::Slave_SendIntArr(int* arr, int n)
{
    MPI_Send(arr, n * sizeof(int), MPI_BYTE, 0, 123, MPI_COMM_WORLD);
}

void Transmitter::Master_CollectIntArr(int* arr, int n, int num_procs)
{
    MPI_Status status;
    int tmp[1024];
    for (int p = 1; p < num_procs; p ++)
    {
        MPI_Recv(tmp, n * sizeof(int), MPI_BYTE, p, 123, MPI_COMM_WORLD, &status);        
        for (int i = 0; i < n; i ++)
            arr[i] += tmp[i];
    }
}